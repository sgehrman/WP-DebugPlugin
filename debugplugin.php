    ...
    if($lic->is_licensed()){
        ...
        <form action="" method="post">
            <table class="form-table">
                <tr>
                    <th style="width:100px;"><label for="input_license_key">License Key</label></th>
                    <td ><input class="regular-text" type="text" id="input_license_key" name="input_license_key"  value="" ></td>
                </tr>
            </table>
            <p class="submit">
...
class LicenseManager{
    ...
    public function __construct($lic=false , $server , $api_key){
        if($this->is_licensed())f
            $this->lic      =   get_option($savedLicenseKey);

        $this->server   =   $server;f
    ...
    public function active(){
        $url = $this->server . '/?secret_key=' . $api_key . '&slm_action=slm_activate&license_key=' . $this->lic . '&registered_domain=' . get_bloginfo('siteurl').'&item_reference='.$this->product_id;
